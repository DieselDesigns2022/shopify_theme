(function () {
  ...
  function initProduct(section) {
    ...
    if (gallery) { var media = Array.prototype.slice.call(gallery.querySelectorAll('[data-product-media]')), thumbs = Array.prototype.slice.call(gallery.querySelectorAll('[data-product-thumbnail]')), initialMediaId = gallery.dataset.initialMediaId, active = media.map(function (item) { return item.dataset.mediaId; }).indexOf(initialMediaId); if (active < 0) active = 0; var modal = gallery.querySelector('[data-product-modal]'), modalImage = gallery.querySelector('[data-product-modal-image]'), close = gallery.querySelector('[data-product-modal-close]'), lastFocus = null;
      function setModalOpen(isOpen) { if (!modal) return; modal.hidden = !isOpen; modal.setAttribute('aria-hidden', isOpen ? 'false' : 'true'); if ('inert' in modal) modal.inert = !isOpen; document.body.classList.toggle('product-modal-open', isOpen); }
      setModalOpen(false);
      ...
      function closeModal() { if (!modal || modal.hidden) return; setModalOpen(false); if (lastFocus && lastFocus.isConnected) lastFocus.focus(); lastFocus = null; }
  ...
  function initOverlay(root) {
    ...
    function setOpenState(isOpen){root.hidden=!isOpen;root.setAttribute('aria-hidden',isOpen?'false':'true');if('inert' in root)root.inert=!isOpen;}
    setOpenState(false);
    function openDialog(){ if(destroyed || !root.isConnected || open) return; setOpenState(true); open=true; if(hasFreshSuccess()) phase14PresentedSuccess[root.id]=true; lockPhase14Dialog(); document.addEventListener('keydown',handleKeydown); if(dialog) dialog.focus(); }
    function closeDialog(){ if(destroyed || !open) { if(!destroyed)setOpenState(false); return; } setOpenState(false); open=false; unlockPhase14Dialog(); document.removeEventListener('keydown',handleKeydown); if(!isAge && !designMode){ phase14VisitDismissals[root.id]=true; if(frequency==='session') safeStore(sessionStorage,key,'1'); else if(frequency==='days') safeStore(localStorage,key,String(Date.now())); } restoreFocus(); }
  ...
  document.addEventListener('shopify:section:select', function (event) {
    ...
    if (popup && event.detail && event.detail.sectionId && popup.dataset.sectionId !== String(event.detail.sectionId)) popup = null;
